...
\begin{tikzpicture}[font=\small,
 cls/.style={draw,align=center,inner sep=3pt,minimum height=6mm},
 att/.style={cls,align=left},
 lab/.style={font=\footnotesize,fill=white,inner sep=1pt,align=center},
...
\node[lab] at (3,-2.4) {monitoredBy 0..*};
\node[lab] at (4.65,-3.65) {monitors 0..*};
...
\node[lab] at (2,-4.35) {positions 0..*};
\node[lab] at (2,-6.05) {target 0..1};
...
\node[font=\small] at (5,-8.25) {(a) References, composition and local attributes};
\end{tikzpicture}

\medskip
\begin{tikzpicture}[font=\small,
 cls/.style={draw,align=center,inner sep=3pt,minimum width=24mm},
 inh/.style={-{Triangle[open,length=2mm,width=2mm]}}]
...
\node[cls,align=left] at (6.2,-1.65) {\textit{\guillemotleft enum\guillemotright}\\Position\\\rule{24mm}{.3pt}\\FAILURE\\STRAIGHT\\DIVERGING};
\node[cls,align=left] at (9.2,-1.65) {\textit{\guillemotleft enum\guillemotright}\\Signal\\\rule{24mm}{.3pt}\\FAILURE\\STOP\\GO};
\node[font=\small] at (3.5,-4) {(b) Shared superclass and enumerations};
\end{tikzpicture}
\end{minipage}
\end{document}
