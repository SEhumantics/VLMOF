...
for stem in railway-batch-1 railway-batch-2 railway-inject-1 railway-inject-2 railway-repair-1 railway-repair-2; do
  ...
  mvn -q -f "$review/pom.xml" exec:java -Dexec.mainClass=org.vlmof.bridge.EmfInterchange -Dexec.args="compare $input/$stem.e1.json $dir/reloaded.e1.json" > "$dir/compare.txt"
