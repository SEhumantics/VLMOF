...
public final class IdentityCorrespondence {
  ...
  private static int id(JsonNode n){if(!n.has("id")||!n.get("id").canConvertToInt())throw new IllegalArgumentException("identity map row lacks integer id");return n.get("id").intValue();}
  ...
  private static Map<Integer,Integer> mapping(String group, JsonNode sourceMap, JsonNode targetMap, JsonNode sourceItems, JsonNode targetItems){
    ...
    for(JsonNode r:sourceMap.path("identities").withArray(group)){int i=id(r);if(sourceByNative.putIfAbsent(identity(r),i)!=null)throw new IllegalArgumentException("duplicate source native identity "+group);sourceIds.add(i);}
    for(JsonNode r:targetMap.path("identities").withArray(group)){int t=id(r);Integer s=sourceByNative.get(identity(r));if(s==null||out.putIfAbsent(t,s)!=null)throw new IllegalArgumentException("missing or duplicate target native identity "+group);targetIds.add(t);}
    Set<Integer> actualS=new HashSet<>(),actualT=new HashSet<>();for(JsonNode x:sourceItems)actualS.add(id(x));for(JsonNode x:targetItems)actualT.add(id(x));
    if(!sourceIds.equals(actualS)||!targetIds.equals(actualT)||out.size()!=actualT.size()||!new HashSet<>(out.values()).equals(actualS))throw new IllegalArgumentException("identity coverage failure "+group); return out;
  }
  ...
  public static JsonNode normalize(JsonNode source,JsonNode reimported,JsonNode map){
    if(!"vlmof-e1-map-1".equals(map.path("version").asText()))throw new IllegalArgumentException("unsupported identity map version"); JsonNode targetMap=reimported.path("provenance").path("identities"); if(targetMap.isMissingNode())throw new IllegalArgumentException("reimported provenance lacks identities"); ObjectNode r=(ObjectNode)reimported.deepCopy(); Map<String,Map<Integer,Integer>> m=new HashMap<>();
    for(String g:GROUPS)m.put(g,mapping(g,map,targetMap,source.path("schema").path(g),r.path("schema").path(g)));
    ObjectNode s=(ObjectNode)r.path("schema"); for(JsonNode x:s.withArray("packages")){ObjectNode o=(ObjectNode)x;set(o,"id",m.get("packages"),"package");if(!o.path("parent").isNull())set(o,"parent",m.get("packages"),"package");}
