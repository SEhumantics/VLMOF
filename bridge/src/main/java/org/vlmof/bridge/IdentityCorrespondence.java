...
public final class IdentityCorrespondence {
  ...
  private static int number(JsonNode n,String label) {
    if(n==null || !n.isIntegralNumber() || !n.canConvertToInt() || n.intValue()<0)
      throw new IllegalArgumentException("identity correspondence requires nonnegative integer " + label);
    return n.intValue();
  }
  private static int id(JsonNode n){return number(n.get("id"),"id");}
  ...
  private static void set(ObjectNode n,String f,Map<Integer,Integer> m,String what){n.put(f,remap(m,number(n.get(f),f),what));}
  ...
  public static JsonNode normalize(JsonNode source,JsonNode reimported,JsonNode map){
    ...
    for(JsonNode x:s.withArray("classes")){ObjectNode o=(ObjectNode)x;set(o,"id",m.get("classes"),"class");set(o,"package",m.get("packages"),"package");ArrayNode a=(ArrayNode)o.get("supers");for(int i=0;i<a.size();i++)a.set(i,com.fasterxml.jackson.databind.node.IntNode.valueOf(remap(m.get("classes"),number(a.get(i),"array ID"),"class")));}
